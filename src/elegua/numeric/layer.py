...
from elegua.comparison import LayerFn, LayerResult
...
def make_numeric_layer(
    *,
    ...
    - ``values`` field → array comparator (array_strategy or strategy)
      (optionally returning structured array diagnostics)
    ...
    array_cmp = (
        make_array_comparator(_arr_tol, k=k, return_diagnostics=return_diagnostics)
        if _arr_tol is not None
        else None
    )
    samples_cmp = make_numeric_comparator(tol=tol, min_samples=min_samples)

    def compare(token_a: ValidationToken, token_b: ValidationToken) -> LayerResult:
        ra = token_a.result
        rb = token_b.result
        ...
        if ra is not None and "values" in ra and rb is not None and "values" in rb:
            ...
            if return_diagnostics or isinstance(result, TaskStatus):
                return result
